/**
 * Madelia - Currency Conversion System
 */

const Currency = {
    baseCurrency: 'INR',
    baseSymbol: '₹',
    currentCountry: localStorage.getItem('madelia_country') || 'India',
    currentCurrency: localStorage.getItem('madelia_currency_code') || 'INR',
    currentSymbol: localStorage.getItem('madelia_currency_symbol') || '₹',
    rates: JSON.parse(localStorage.getItem('madelia_exchange_rates')) || { INR: 1 },
    lastFetched: localStorage.getItem('madelia_rates_timestamp') || 0,
        ...
        if (now - this.lastFetched > 24 * 60 * 60 * 1000) {
            try {
                ...
                if (response.ok) {
                    ...
                    localStorage.setItem('madelia_exchange_rates', JSON.stringify(this.rates));
                    localStorage.setItem('madelia_rates_timestamp', now);
                }
            } catch (err) {
    ...
    setCountry(countryName) {
        ...
        localStorage.setItem('madelia_country', this.currentCountry);
        localStorage.setItem('madelia_currency_code', this.currentCurrency);
        localStorage.setItem('madelia_currency_symbol', this.currentSymbol);
    ...
    formatPrice(priceString) {
        ...
        if (typeof priceString === 'number') {
            ...
            rawPrice = parseFloat(priceString.toString().replace(/[^\d.-]/g, '')) || 0;
        }
