...
const ProductDetailHandler = {
    ...
    async fetchProduct() {
        if (!this.product && window.Site && window.Site.products && window.Site.products.length > 0) {
            this.product = window.Site.products.find(p => (p._id || p.id)?.toString() === this.productId);
        }
        
        // If still not found, fetch it directly (do not depend on window.Site)
        if (!this.product) {
            try {
                const ts = new Date().getTime();
                const res = await fetch(`${API_BASE_URL}/products/${this.productId}?t=${ts}`);
                if (res.ok) {
    ...
    render() {
        ...
        let images = product.image_path ? [product.image_path] : (product.image ? [product.image] : ['assets/Logo/madelia%20logo.png']);
        let colorsHtml = '';
        let sizesHtml = '';
        ...
        if (product.variants) {
            if (Array.isArray(product.variants)) {
                ...
                colors = Object.keys(product.variants)
                    ...
                    .map(k => product.variants[k])
                    .filter(v => v && v.color);
                    
                if (product.variants._sizes) {
                    sizes = product.variants._sizes;
                }
            }
        }

        if (colors.length > 0) {
            images = colors.map(v => v.image);
            colorsHtml = `
                    ...
                    <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                        ${colors.map((v, idx) => `
                            <button onclick="ProductDetailHandler.setColor(${idx})" 
                                style="padding: 8px 15px; border: 2px solid ${this.selectedColorIdx === idx ? '#C8A89A' : '#eee'}; 
                                border-radius: 4px; background: white; cursor: pointer; font-weight: 500; transition: all 0.2s;
                                color: ${this.selectedColorIdx === idx ? '#C8A89A' : '#334155'};">
                                ${v.color}
                            </button>
                        `).join('')}
                    </div>
        ...
        if (sizes.length > 0) {
            if (this.selectedSize === null) this.selectedSize = sizes[0];
            sizesHtml = `
                    ...
                    <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                        ${sizes.map(size => `
                            <button onclick="ProductDetailHandler.setSize('${size}')" 
                                style="padding: 8px 15px; border: 2px solid ${this.selectedSize === size ? '#C8A89A' : '#eee'}; 
                                border-radius: 4px; background: white; cursor: pointer; font-weight: 500; transition: all 0.2s;
                                color: ${this.selectedSize === size ? '#C8A89A' : '#334155'};">
                                ${size}
                            </button>
                        `).join('')}
                    </div>
                        ...
                        ${this.getReviews().length === 0 ? '<p style="color:#94a3b8;">No reviews yet. Be the first to review this product!</p>' : 
                          this.getReviews().map(r => `
                            <div class="review-item">
                                ${r.image_url ? `<img src="${r.image_url}" class="review-img" alt="Review Image">` : ''}
                                <div class="review-content">
                                    <div class="review-stars">${'★'.repeat(r.rating)}${'☆'.repeat(5-r.rating)}</div>
                                    <div class="review-customer">${r.name}</div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    .btn-write-review { background: #C8A89A; color: #fff; border: none; padding: 0.5rem 1rem; border-radius: 4px; cursor: pointer; font-weight: 500; transition: background 0.2s; }
    ...
    .review-stars { color: #C8A89A; margin-bottom: 0.5rem; font-size: 1.1rem; }
