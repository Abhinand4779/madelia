/**
 * Madelia - Authentication and State Management
...
const Auth = {
    ...
    slimProduct(product) {
        return {
            ...
            image: product.image_path || (product.variants && product.variants.length > 0 ? product.variants[0].image : null) || product.image || (product.images && product.images[0]) || null,
    ...
    async placeOrder(shippingInfo, totalAmount = null) {
        ...
        try {
            ...
            if (res.ok) {
                ...
                // Add full items data to order for immediate display purposes (backend may not return full item details)
            ...
            this.userOrders.unshift(order);
            const allOrders = JSON.parse(localStorage.getItem('madelia_orders') || '[]');
            allOrders.unshift(order);
            localStorage.setItem('madelia_orders', JSON.stringify(allOrders));
            this.clearCart();
            return order;
        }
    },
