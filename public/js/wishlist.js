                        ...
                        ${Auth.wishlist.map(item => `
                            <div class="wishlist-card">
                                <div class="wishlist-card-img" onclick="window.location.href='product.html?id=${item._id || item.id}'">
                                    <img src="${item.image_path || (item.variants && item.variants.length > 0 ? item.variants[0].image : '') || item.image || item.images?.[0] || 'assets/Logo/madelia%20logo.png'}" alt="${item.name}">
                                ...
                                <div class="wishlist-card-info">
                                    <p class="wishlist-category">${item.category ? (item.category.name || item.category) : 'Uncategorized'}</p>
