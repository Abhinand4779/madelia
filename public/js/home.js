...
const HomeHandler = {
        ...
        if (hasSliders) {
            ...
            wrap.innerHTML = `
                ...
                <div class="slider-dots" style="position: absolute; bottom: 20px; left: 50%; transform: translateX(-50%); display: ${sliders.length > 1 ? 'flex' : 'none'}; gap: 10px; z-index: 10;">
                    ${sliders.map((_, index) => `
                        <button style="width: 12px; height: 12px; border-radius: 50%; border: none; background-color: ${index === current ? '#C8A89A' : 'rgba(255,255,255,0.5)'}; cursor: pointer; transition: all 0.3s;" onclick="HomeHandler.setHero(${index})"></button>
                    `).join('')}
                </div>
            `;
        } else {
                ...
                ${config.promoCarousel.map((slide, index) => `
                    <div class="promo-slide ${index === current ? 'promo-slide--active' : ''} ${slide.align === 'right' ? 'promo-slide--reverse' : ''}">
                        ...
                        <div class="promo-slide__content">
                            <p class="promo-slide__eyebrow">Madelia — Featured</p>
    ...
    renderCategories(config) {
        ...
        if (!wrap) return;
        this.fetchAndRenderLiveCategories(wrap, config);
    },

    async fetchAndRenderLiveCategories(wrap, config) {
        const SERVER = '';
        const API    = '/api';
        let liveCategories = [];
        ...
        try {
            ...
            if (res.ok) liveCategories = await res.json();
        } catch (e) {}

        if (liveCategories.length > 0) {
            wrap.innerHTML = liveCategories
                .filter(cat => !cat.parent_id)
                .map(cat => {
                    const imgSrc = cat.image_url
                        ? (cat.image_url.startsWith('http') ? cat.image_url : `${SERVER}${cat.image_url}`)
                        : null;
                    return `
                        <div class="futuristic-cat-card" onclick="HomeHandler.handleCategoryClick('${cat.name}')" style="cursor:pointer;">
                            ${imgSrc
                                ? `<img src="${imgSrc}" alt="${cat.name}" style="width:100%;height:80%;object-fit:cover;">`
                                : `<div style="width:100%;height:80%;background:linear-gradient(135deg,#ffffff,#1a3a52);display:flex;align-items:center;justify-content:center;"><i class="bi bi-bag" style="font-size:2.5rem;color:rgba(200, 168, 154,0.5);"></i></div>`
                            }
            ...
            // Fallback to config
            const cats = config.homeCategories || [];
            if (!cats.length) return;
            wrap.innerHTML = cats.map(cat => `
                <div class="futuristic-cat-card" onclick="HomeHandler.handleCategoryClick('${cat.name}')">
