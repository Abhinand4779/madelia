...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Mock Data Fallback
    let brands = [
        { id: 1, name: 'Madelia Premium', image: '../assets/Logo/madelia%20logo.png', status: 'active' }
    ];
        ...
        filtered.forEach((brand, index) => {
            ...
            tableBody.innerHTML += `
                <tr>
                    <td>${index + 1}</td>
                    <td>${brand.name}</td>
                    <td><img src="${brand.image || '../assets/Logo/madelia%20logo.png'}" alt="${brand.name}" class="cat-thumbnail"></td>
    ...
    brandForm.addEventListener('submit', async (e) => {
        ...
        const newBrand = {
            ...
            name: name,
            status: status,
            image: '../assets/Logo/madelia%20logo.png' // Mock upload
        };
