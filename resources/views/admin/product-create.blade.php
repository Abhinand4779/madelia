    ...
    <img class="mx-auto h-11 w-11 transition-transform duration-500 ease-in-out hover:rotate-[360deg]" src="/assets/Logo/madelia%20logo.png" alt="logo">
        ...
        <div class="flex flex-col pt-6 space-y-4 overflow-y-auto is-scrollbar-hidden grow">
          <!-- Dashobards -->
                    ...
                    <a href="/admin/dashboard" class="flex h-11 w-11 items-center justify-center rounded-lg bg-primary/10 text-primary outline-none transition-colors duration-200 hover:bg-primary/20 focus:bg-primary/20 active:bg-primary/25 dark:bg-navy-600 dark:text-accent-light dark:hover:bg-navy-450 dark:focus:bg-navy-450 dark:active:bg-navy-450/90" x-tooltip.placement.right="'Dashboard'">
...
<li>
            <a href="/admin/dashboard" class="flex py-2 text-xs+ tracking-wide outline-none transition-colors duration-300 ease-in-out text-slate-600 hover:text-slate-900 dark:text-navy-200 dark:hover:text-navy-50">
                Dashboard
            </a>
        </li>
<li>
            <a href="/admin/categories" class="flex py-2 text-xs+ tracking-wide outline-none transition-colors duration-300 ease-in-out text-slate-600 hover:text-slate-900 dark:text-navy-200 dark:hover:text-navy-50">
                Category
            </a>
        </li>
<li>
            <a href="/admin/subcategories" class="flex py-2 text-xs+ tracking-wide outline-none transition-colors duration-300 ease-in-out text-slate-600 hover:text-slate-900 dark:text-navy-200 dark:hover:text-navy-50">
                Sub Category
            </a>
        </li>
<li>
            <a href="#" class="flex py-2 text-xs+ tracking-wide outline-none transition-colors duration-300 ease-in-out text-slate-600 hover:text-slate-900 dark:text-navy-200 dark:hover:text-navy-50">
                Child Category
            </a>
        </li>
<li>
...
<li>
            <a href="/admin/product/list" class="flex py-2 text-xs+ tracking-wide outline-none transition-colors duration-300 ease-in-out text-slate-600 hover:text-slate-900 dark:text-navy-200 dark:hover:text-navy-50">
                Product Lists
            </a>
        </li>
<li>
            <a href="/admin/product/create-or-update" class="flex py-2 text-xs+ tracking-wide outline-none transition-colors duration-300 ease-in-out font-medium text-primary dark:text-accent-light">
                Create Product
            </a>
        </li>
<li>
        ...
        .btn-apply {
            ...
            font-size: 0.9rem;
                ...
                <div class="ads-header-row">
                    ...
                    <div class="ads-actions"><a href="/admin/product/list" style="color: #666; text-decoration: none; display: flex; align-items: center; gap: 5px;"><i class="fa fa-arrow-left"></i> Back to Products</a></div>
                </div>

                <div class="ads-table-card">
                    <form id="product-form" style="padding: 20px;">
                        <div style="margin-bottom: 20px;">
                            <label style="display: block; margin-bottom: 8px; font-weight: 500;">Product Name</label>
                            <input type="text" id="p-name" required placeholder="e.g., Silk Saree" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px;">
                        </div>
                        <div style="margin-bottom: 20px;">
                            <label style="display: block; margin-bottom: 8px; font-weight: 500;">Price (INR)</label>
                            <input type="number" id="p-price" required placeholder="e.g., 5000" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px;">
                        </div>
                        <div style="margin-bottom: 20px;">
                            <label style="display: block; margin-bottom: 8px; font-weight: 500;">Sizes Available (Comma separated, e.g., S, M, L)</label>
                            <input type="text" id="p-sizes" placeholder="e.g. S, M, L" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px;">
                        </div>
                        <div style="margin-bottom: 20px;">
                            <label style="display: block; margin-bottom: 8px; font-weight: 500;">Category</label>
                            <select id="p-cat" required style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px;"></select>
                        </div>
                        <div style="margin-bottom: 20px;">
                            <label style="display: block; margin-bottom: 8px; font-weight: 500;">Subcategory (Optional)</label>
                            <select id="p-subcat" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px;">
                                <option value="">None</option>
                            </select>
                        </div>
                        <div style="margin-bottom: 20px;">
                            <label style="display: block; margin-bottom: 8px; font-weight: 500;">Product Image Variants (Add Colors)</label>
                            <div id="variants-container" style="display: flex; flex-direction: column; gap: 15px;">
                                <!-- Dynamic variants will be added here -->
                            </div>
                            <button type="button" onclick="addVariantField()" style="margin-top: 10px; padding: 8px 15px; background: #3b82f6; color: white; border: none; border-radius: 4px; cursor: pointer;">+ Add Color Variant</button>
                        </div>
                        <button type="submit" id="submit-btn" style="background: #1a202c; color: #fff; border: none; padding: 12px 20px; border-radius: 5px; cursor: pointer; font-weight: bold; width: 100%;">Save Product</button>
                    </form>
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", async () => {
        ...
        const subcatSelect = document.getElementById("p-subcat");
        ...
        try {
            ...
            if (res.ok) {
                ...
                catSelect.innerHTML = '<option value="">Select Category</option>' + apiCategories.map(c => `<option value="${c.id}">${c.name}</option>`).join('');
            }
        ...
        catSelect.addEventListener('change', () => {
            ...
            const catObj = apiCategories.find(c => c.id == selectedCatId);
            if(catObj && catObj.children && catObj.children.length > 0) {
                subcatSelect.innerHTML = '<option value="">None</option>' + catObj.children.map(s => `<option value="${s.id}">${s.name}</option>`).join('');
        ...
        if (editId) {
            ...
            try {
                ...
                if(res.ok) {
                    ...
                    if(document.getElementById('p-desc')) document.getElementById('p-desc').value = product.description || '';
                    document.getElementById('p-price').value = product.price || '';
                    
                    if (product.variants && product.variants._sizes) {
                        document.getElementById('p-sizes').value = product.variants._sizes.join(', ');
                    }
                    
                    catSelect.value = product.category_id || '';
                    catSelect.dispatchEvent(new Event('change'));
                    
                    // Note: We don't have a direct subcategory field on product schema, so we skip setting subcat for now unless it's stored in variants.
                    ...
                    Object.keys(variants).forEach(k => {
                        if (k !== '_sizes') {
                            hasVariants = true;
                            addVariantField(variants[k].color || '', variants[k].image || '');
                        }
                    });
                    
                    if (hasVariants) {
                        document.getElementById('variants-container').innerHTML = ''; // Clear default
                        Object.keys(variants).forEach(k => {
                            if (k !== '_sizes') addVariantField(variants[k].color || '', variants[k].image || '');
                        });
                    } else if (product.image_path) {
                        document.getElementById('variants-container').innerHTML = '';
                        addVariantField('Default', product.image_path);
                    }
                }
            } catch(e) { console.error('Failed to load product for edit', e); }
        }
    });

    let productVariants = [];

    function addVariantField(color = '', imageBase64 = '') {
        const container = document.getElementById('variants-container');
        const variantId = Date.now() + Math.floor(Math.random() * 1000);
        const imgDisplay = imageBase64 ? 'block' : 'none';
        const requiredAttr = imageBase64 ? '' : 'required';
        const html = `
            <div class="variant-item" id="variant-${variantId}" style="padding: 15px; border: 1px solid #eee; border-radius: 5px; background: #f9fafb;">
                <div style="display: flex; gap: 10px; margin-bottom: 10px;">
                    <input type="text" class="variant-color" value="${color}" placeholder="Color Name (e.g. Red, Gold)" required style="flex: 1; padding: 10px; border: 1px solid #ddd; border-radius: 4px;">
                    <button type="button" onclick="document.getElementById('variant-${variantId}').remove()" style="padding: 10px; background: #dc3545; color: white; border: none; border-radius: 4px; cursor: pointer;"><i class="fa fa-trash"></i></button>
                </div>
                <input type="file" class="variant-image" accept="image/*" ${requiredAttr} data-base64="${imageBase64}" style="width: 100%; padding: 10px; border: 1px solid #ddd; border-radius: 4px;" onchange="handleVariantImage(this, ${variantId})">
                <img id="variant-preview-${variantId}" src="${imageBase64}" style="width: 100px; height: 100px; object-fit: contain; display: ${imgDisplay}; margin-top: 10px; border: 1px solid #ddd;">
            </div>
        `;
        container.insertAdjacentHTML('beforeend', html);
    }

    // Add first variant by default if not editing
    if (!window.location.search.includes('edit=')) {
        addVariantField();
    }

    function handleVariantImage(input, id) {
        ...
        if (file) {
            ...
            reader.onload = function(event) {
                ...
                img.onload = function() {
                    ...
                    input.dataset.base64 = base64; // Store in dataset
                    
                    const preview = document.getElementById(`variant-preview-${id}`);
                    preview.src = base64;
    ...
    document.getElementById('product-form').addEventListener('submit', async function(e) {
        ...
        variantItems.forEach((item) => {
            ...
            const imgInput = item.querySelector('.variant-image');
            const base64 = imgInput.dataset.base64;
            
            if(color && base64) {
                variantsData.push({ color: color, image: base64 });
            }
        });

        const data = {
            name: document.getElementById('p-name').value,
            description: document.getElementById('p-desc') ? document.getElementById('p-desc').value : '',
            price: document.getElementById('p-price').value,
            sizes: document.getElementById('p-sizes').value ? document.getElementById('p-sizes').value.split(',').map(s => s.trim()).filter(s => s) : [],
            category_id: document.getElementById('p-cat').value,
            category: document.getElementById('p-cat').value,
            subcategory: document.getElementById('p-subcat') ? document.getElementById('p-subcat').value : '',
            variants: variantsData,
            image_path: variantsData.length > 0 ? variantsData[0].image : '',
            image: variantsData.length > 0 ? variantsData[0].image : ''
        };
        ...
        try {
            const url = editId ? `${API_BASE_URL}/admin/products/${editId}` : `${API_BASE_URL}/admin/products`;
